...
static void count_classes(node_id *cls, node_id n, node_id *cnt);
  ...
  { if (!g1->CompatibleNode(g1->GetNodeAttr(node1),
    ...
    for(i=0; i<g2->OutEdgeCount(node2); ++i )
      { o2=g2->GetOutEdge(node2, i, &attr2);
		  // check for loops (mapping via core_* not yet stored)
		if (node2 == o2) {
			  // check if a corresponding loop is present in g1 too
			if (!g1->HasEdge(node1, node1) ||
				!g1->CompatibleEdge(g1->GetEdgeAttr(node1, node1), attr2))
			  return false;
		} else
