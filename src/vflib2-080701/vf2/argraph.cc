...
template <class T>
inline void grow(T* &ptr, SIZETYPE old_size, SIZETYPE new_size)
...
template <class T>
inline void clear(T* ptr, SIZETYPE n)
  { SIZETYPE i;
    for(i=0; i<n; ++i )
      ptr[i]=(T)0;
  }

inline void clear(byte *ptr, SIZETYPE n)
  { memset(ptr, 0, n);
  }
...
ARGraph_impl::ARGraph_impl(ARGLoader *loader)
  { node_destroyer=NULL;
    ...
    clear(out_attr, n);
    for(i=0; i<n; ++i )
      { node_id k=out_count[i]=loader->OutEdgeCount(i);
        out[i]=new node_id[k];
        ptrcheck(out[i]);
