...
bool VF2MonoState::IsFeasiblePair(node_id node1, node_id node2)
  { assert(node1<n1);
    ...
    SIZETYPE i;
    node_id other1, other2;
    void *attr1;
    node_id termout1=0, termout2=0, termin1=0, termin2=0, new1=0, new2=0;

    // Check the 'out' edges of node1
    for(i=0; i<g1->OutEdgeCount(node1); ++i )
      { other1=g1->GetOutEdge(node1, i, &attr1);
		  // check for loops (mapping via core_* not yet stored)
		if (node1 == other1) {
			  // check if a corresponding loop is present in g2 too
            if (!g2->HasEdge(node2, node2) ||
                !g1->CompatibleEdge(attr1, g2->GetEdgeAttr(node2, node2)))
              return false;
		} else
...
// ##### the following check is obsolete since each loop in edge is already handled as a loop out edge
//		  // check for loops (mapping via core_* not yet stored)
//		if (node1 == other1) {
//			  // check if a corresponding loop is present in g2 too
//			if (!g2->HasEdge(node2, node2) ||
...
//    int i, other;
    node_id i, other;
    for(i=0; i<(node_id)g1->InEdgeCount(node1); ++i )
    ...
    for(i=0; i<(node_id)g1->OutEdgeCount(node1); ++i )
    ...
    for(i=0; i<(node_id)g2->InEdgeCount(node2); ++i )
    ...
    for(i=0; i<(node_id)g2->OutEdgeCount(node2); ++i )
...
//		  { int other=g1->GetInEdge(added_node1, i);
		  { node_id other=g1->GetInEdge(added_node1, i);
...
//		  { int other=g1->GetOutEdge(added_node1, i);
		  { node_id other=g1->GetOutEdge(added_node1, i);
...
//		  { int other=g2->GetInEdge(node2, i);
		  { node_id other=g2->GetInEdge(node2, i);
...
//		  { int other=g2->GetOutEdge(node2, i);
		  { node_id other=g2->GetOutEdge(node2, i);
