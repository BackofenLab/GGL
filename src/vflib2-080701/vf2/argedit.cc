 ...
 *
 * Author: P. Foggia
 * $Id: argedit.cc,v 1.5 2011/11/19 23:05:53 mmann Exp $
 -----------------------------------------------------------*/
 ...
 * Returns the number of edges coming out of a node.
 ----------------------------------------------------*/
SIZETYPE ARGEdit::OutEdgeCount(node_id id)
  { nNode *n=nodes;
 ...
 * Returns an edge
 ----------------------------------------------------*/
node_id ARGEdit::
GetOutEdge(node_id id, SIZETYPE i, void **pattr)
  { nNode *n=nodes;
...
void ARGEdit::InsertEdge(node_id id1, node_id id2, void* attr)
  {
    ...
    if (pn==NULL || pn->id!=id1)
      error("Bad param 1 in ARGEdit::InsertEdge: from node %d does not exist", (SIZETYPE)id1);
    ...
    if (p!=NULL && p->to==id2)
      error("Bad param 2 in ARGEdit::InsertEdge: edge (%d,%d) exists", (SIZETYPE)id1, (SIZETYPE)id2);
