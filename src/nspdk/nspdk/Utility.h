#ifndef SGM_NSPDK_UTILITY_H
#define SGM_NSPDK_UTILITY_H
...
#elif HAVE_TR1_UNORDERED_MAP > 0
		std::tr1::unordered_map<unsigned, std::vector<unsigned> >
#else
...
typedef
#if HAVE_UNORDERED_MAP > 0
		std::unordered_map< std::pair<unsigned, unsigned>, int>
#elif HAVE_TR1_UNORDERED_MAP > 0
		std::tr1::unordered_map< std::pair<unsigned, unsigned>, int>
#else
		std::map< std::pair<unsigned, unsigned>, int>
#endif
	umap_pair_uint_uint_int;

typedef
#if HAVE_UNORDERED_MAP > 0
		std::unordered_map< std::pair<unsigned, int>, std::vector<unsigned> >
#elif HAVE_TR1_UNORDERED_MAP > 0
		std::tr1::unordered_map< std::pair<unsigned, int>, std::vector<unsigned> >
#else
		std::map< std::pair<unsigned, int>, std::vector<unsigned> >
#endif
	umap_pair_uint_int_vec_uint;
...
inline int IntHashPair(int key1, int key2, int aModulo=2147483647)
{
	...
	int key=(int)((double)key1*(double)(key2+1)*A);
...
inline int IntHash(int key, int aModulo, unsigned aSeed){
  const double A=sqrt(2)-1;
  return IntHashSimple((int)((double)key*(double)(aSeed+1)*A),aModulo);
}
