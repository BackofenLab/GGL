...
public:
  ...
  void Usage(string aCommandName){
	...
	<<"[-np <num parts to replicate edge induced graphs> (default: 6)]"<<endl
	<<"[-snlp <num of low parts to skip> (default: 2)]"<<endl
  ...
  void Init(int argc, char** argv){
	...
	else if ((*it)=="-np") mNumParts=stream_cast<unsigned>(*(++it)); 
	else if ((*it)=="-snlp") mSkipNumParts=stream_cast<unsigned>(*(++it)); 
...
class ClusterClass{
public:
  ClusterClass(NSPDK_FeatureGenerator* paFeatureGenerator, double aAlpha, unsigned aNumParts, unsigned aSkipNumParts, bool aMarkParts):
    pmFeatureGenerator(paFeatureGenerator), 
    mAlpha(aAlpha),
    mNumParts(aNumParts),
    mSkipNumParts(aSkipNumParts),
  ...
  void Input(const string& aInputFileName){
    ...
    while (!fin.eof()){
      ...
      //extract sorted list of edge weight values (exclude backbone)
      vector<double> edge_weight_list;
      for (unsigned u=0;u<G.VertexSize();++u){
	vector<unsigned> adj=G.GetVertexAdjacentList(u);
	for (unsigned vid=0;vid<adj.size();vid++){
	  unsigned v=adj[vid];
	  double weight=G.GetEdgeNumericAttributeList(u,v,0);
	  string edge_label=G.GetEdgeSymbolicAttributeList(u,v,0);
	  if (edge_label!=">") //NOTE: backbone edges are marked with symbol ">"
	    edge_weight_list.push_back(weight);
	}
      }
      sort(edge_weight_list.begin(),edge_weight_list.end());
	
      //induce graphs for several thresholds values
      SVector x;
      for (unsigned t=mSkipNumParts;t<mNumParts-mSkipNumParts;t++){
	unsigned edge_weight_list_index=(unsigned)((double)t*(double)edge_weight_list.size()/(double)mNumParts);
	double threshold_value=edge_weight_list[edge_weight_list_index];
	GraphClass G_t=EdgeInduceByEdgeLabelValue(G, threshold_value);
	SVector x_t;
	pmFeatureGenerator->generate_feature_vector(G_t,x_t);
	x.add(x_t);
      }
...
void SetGraphFromFileGSPAN(istream& in, GraphClass& oG){
  ...
  while(!in.eof() && in.good() && in.peek()!='t' && getline(in,line)){//read until next 't' or end of file
    ...
    } else throw logic_error("Bad format");
  }
}

GraphClass EdgeInduceByEdgeLabelValue(const GraphClass& aG, double aThresholdValue){
  GraphClass G;
  //copy all vertices
  for (unsigned u=0;u<aG.VertexSize();u++){
    vector<string> vertex_symbolic_attribute_list=aG.GetVertexSymbolicAttributeList(u);
    vector<bool> vertex_status_attribute_list=aG.GetVertexStatusAttributeList(u);
    unsigned real_vertex_index=G.InsertVertex();
    G.SetVertexSymbolicAttributeList(real_vertex_index, vertex_symbolic_attribute_list);    
    G.SetVertexStatusAttributeList(real_vertex_index, vertex_status_attribute_list);    
  }
  //copy edges only if numerical value is not lower than aThresholdValue
  //do not copy numerical attributes
  //mark dirrently the 's' edges if mMarkParts option is activated
  unsigned counter=0;
  for (unsigned e=0;e<aG.EdgeSize();e++){
    unsigned src=aG.GetEdgeSource(e);
    unsigned dest=aG.GetEdgeDestination(e);
    vector<string> edge_symbolic_attribute_list=aG.GetEdgeSymbolicAttributeList(src,dest);
    if (mMarkParts){
      string edge_label=edge_symbolic_attribute_list[0];
      edge_label+="_"+stream_cast<string>(aThresholdValue);
      edge_symbolic_attribute_list[0]=edge_label;
    }
    vector<double> edge_numeric_attribute_list=aG.GetEdgeNumericAttributeList(src,dest);
    double weight=edge_numeric_attribute_list[0];
    if (weight>=aThresholdValue){
      counter++;
      unsigned edge_index=G.InsertEdge(src,dest);
      unsigned reverse_edge_index=G.InsertEdge(dest,src);
      G.SetEdgeSymbolicAttributeList(edge_index,edge_symbolic_attribute_list);
      G.SetEdgeSymbolicAttributeList(reverse_edge_index,edge_symbolic_attribute_list);
    }
  }
  return G;
}
  ...
  void OutputKNearestNeighbors(unsigned aNumNearestNeighbors, string aMode, ostream& out){
    unsigned effective_num_ranked=min(mDataset.size(),aNumNearestNeighbors);
    for (unsigned i=0;i<mDataset.size();++i){
...
int main(int argc, char** argv){
  ...
  try
  {
    ...
    ClusterClass C(pfg,P.mAlpha,P.mNumParts,P.mSkipNumParts,P.mMarkParts);
