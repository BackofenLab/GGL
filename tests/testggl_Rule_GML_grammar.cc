...
const std::string Rule4 = std::string("\
  3(A)    1(B)     ==>       3(C) -1- 1(D) \n\
");

const std::string Rule4gml = std::string(
"rule [\n\
        ruleID \"Rule4gml\"\
        left [  \n\
                node [ id 3 label \"A\" ]  \n\
...
int main( int argc, char** argv ) {
	...
	for (size_t i=0; i<rules.size(); i++) {
		...
		try {
			std::pair<ggl::Rule, int> ret = ggl::Rule_GML_grammar::parseRule( rulesGML[i] );
